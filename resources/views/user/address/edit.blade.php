...
@section('title', 'My Account | Edit Address')
<style type="text/css">
   .error
   {
      color:#ff0000;
	...
	<div class="container">
        ...
        <div class="col-sm-9">
            <div class="invoice p-3 mb-3">
              <!-- title row -->
              <div class="row">
               <form action="{{ route('address.update', $address->id) }}" method="POST" id="submit-form">
                 {{ csrf_field() }}
                 {{ method_field('PATCH') }}
                <div class="col-md-6">
                     ...
                     <div class="form-group">
                        <label>Full Name: </label>
                         <input name="fullname" type="text" placeholder="ex. Juan Dela Cruz" class="form-control" value="{{ $address->fullname}}">
                     </div>
                     <div class="form-group">
                        <label>Contact No.:</label>
                         <input type="text" name="telephone" class="form-control" placeholder="ex. 09876543210" value="{{ $address->telephone}}">
                     </div>
                     <div class="form-group">
                        <label>Address Description (Note).:</label>
                        <textarea class="form-control textarea-data" name="address_description" placeholder="Please Enter your address description (optional)">{{ $address->address_description}}</textarea>
                     </div>
                </div>
                <div class="col-md-6">
                     <div class="form-group">
                           <label>House/Block/Lot No./Street/Purok/Subd.:</label>
                           <input type="text" name="house_info" class="form-control" placeholder="ex. #03 Coolidge St., Village, Phase 2" value="{{ $address->house_info}}">
                     </div>
                     <div class="form-group">
                        <label>Province:</label>
                           <select class="form-control" name="province_id"  id="province_id">
                              ...
                              <option value="{{ $x->province_id }}" {{ $x->province_id == $address->province_id ? 'selected' : '' }}>{!! $x->name !!}</option>
                              @endforeach
                           </select>
                     </div>
                     <div class="form-group">
...
<script type="text/javascript">
    $(function() { 
      ...
      var address = {
             ...
             barangay: "0{{ $address->barangay_id }}",
             old_city: "0{{ $address->city_id }}"
         };
      $('#province_id').on('change', function () {
            $.getJSON('/api/city/' + this.value, function (data) {
                var options = '<option disabled>Select City</option>';
                $.each(data, function (index, data) {
                  var selected = '';
                  if(data.city_id == address.old_city)
                  {
                  options += '<option value="' + data.city_id + '" selected>' + data.name + '</option>';
                  }
                  options += '<option value="' + data.city_id + '" >' + data.name + '</option>';
